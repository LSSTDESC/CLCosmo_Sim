...
def compute_cluster_lensing(R, cM, logm_grid, z_grid, cosmo_ccl, cosmo_clmm, 
    ...
    def excess_surface_density_mis_fct(R, logm, concentration, z):
            R_Sigma = np.logspace(np.log10(0.0001), np.log10(32), 500)
            Sigma_mis = clmm.compute_surface_density(R_Sigma, 10**logm, concentration, z, 
                                                        cosmo_clmm, delta_mdef=200, halo_profile_model='nfw', massdef='critical')
    ...
    def excess_surface_density_fct(R, logm, concentration, z):
    ...
    if not miscentering:
        for i in range(len(logm_grid)):
            for j in range(len(z_grid)):
                cluster_lensing_grid[:,i,j] = excess_surface_density_fct(R, logm_grid[i], concentration_grid[i,j], z_grid[j])
        ...
        if multiprocessing:
            ...
            def excess_surface_density_mis_fct_n(n):
                return excess_surface_density_mis_fct(R, Logm_grid_flatten[n], Concentration_grid_flatten[n], Z_grid_flatten[n])[0]
            ...
            for i in range(len(logm_grid)):
                for j in range(len(z_grid)):
                    excess_surface_density = excess_surface_density_fct(R, logm_grid[i], concentration_grid[i,j], z_grid[j])
            ...
            for i in range(len(logm_grid)):
                for j in range(len(z_grid)):
                    excess_surface_density = excess_surface_density_fct(R, logm_grid[i], concentration_grid[i,j], z_grid[j])
                    ti = time.time()
                    excess_surface_density_mis, surface_density_mis = excess_surface_density_mis_fct(R, logm_grid[i], concentration_grid[i,j], z_grid[j])
                    tf = time.time()
